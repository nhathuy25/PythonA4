...
def AjouterSeance():
    global entreeMatiere, entreeDate, col

    matiere = entreeMatiere.get().strip()
...
#Ajouter Seance
labelMatiere= tk.Label(master=frame, text="Nom de matière:")
labelMatiere.place(x=startX+2.3*offsetX1, y=startY + offsetY)

entreeMatiere = tk.Entry(master=frame)
entreeMatiere.place(x=startX+offsetX1*4, y=startY + offsetY)
